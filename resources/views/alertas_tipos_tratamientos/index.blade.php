<x-app-layout title="Tipos de Tratamientos" :breadcrumbs="[['title' => 'Inicio', 'url' => '/dashboard'],['title' => 'Alertas', 'url' => '/alertasIndex']]">
	<x-slot name="header">
		<h2 class="font-semibold text-xl text-gray-800 leading-tight">
			{{ __('Tipos de Tratamientos') }}
		</h2>
	</x-slot>
		...
		jQuery(document).ready(function($) {
			...
			*******************************************************************************************************************************/
			$("#form").submit(function(e) {
				...
				//show_loading();
				$.ajax({
					url: "{{ url('alertas_tipos_tratamientos/ajax_store') }}",
					type: "POST",
					data: formData,
					method: 'POST',
					cache: false,
			...
			table = $('#alertas_tipos_table').DataTable({
				"ajax": {
					url: "{{ url('alertas_tipos_tratamientos/ajax_listado') }}",
					type: 'GET',
 ...
 */				},
		...
		/*******************************************************************************************************************************
		 *******************************************************************************************************************************/
		function edit_alertas_tipos_tratamientos(id) {
			save_method = 'update';
			$('#form')[0].reset();
			$('.form-group').removeClass('has-error');
			$('.help-block').empty();
			...
			$.ajax({
				url: "{{ route('alertas_tipos_tratamientos.ajax_edit', ':id') }}".replace(':id', id),
				type: "GET",
				dataType: "JSON",
				success: function(data) {
		...
		/*******************************************************************************************************************************
		 *******************************************************************************************************************************/
		function delete_alertas_tipos_tratamientos(id) {
			if (confirm('¿Desea borrar este tipo de tratamiento?')) {

				$.ajax({
					url: "{{ route('alertas_tipos_tratamientos.ajax_delete', ':id') }}".replace(':id', id),
					type: "POST",
					dataType: "JSON",
					headers: {
					...
					success: function(data) {
						swal.fire("Aviso", "Tipo de Tratamiento eliminado con éxito.", "success");
				...
				<div class="table-responsive">
					<div class="d-flex mb-2">
						<button id="agregar" class="btn btn-success mr-2" onclick="add_alertas_tipos()">
							...
							</svg> {{ __('Agregar Tipo de Tratamiento') }}
						</button>
					</div>
