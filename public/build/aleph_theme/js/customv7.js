...
function validar_columna_importador(){
    ...
    //        return true;
    //    }else{
    //        swal.fire("Aviso", "Carácter no permitido","warning");
    //        return false;
    //    }
    //});
};
...
function validar_solo_letras_y_numeros(event){
    ...
    if((key >= 48 && key <= 57) || (key >= 65 && key <= 90) || (key >= 97 && key <= 122) || key == 8 || key == 32 || key == 16){
        ...
        swal.fire("Aviso", "Caracter no permitido","warning");
...
function validar_solo_letras_palabra(input){
    ...
    for(var i = 0; i < palabra.length; i++){
        ...
        if((code >= 65 && code <= 90) || (code >= 97 && code <= 122) || code == 32){
            ...
            swal.fire("Aviso", "Caracter '"+palabra.charAt(i)+"' no permitido","warning");
            event.preventDefault();
            $(input).val('');['tipo']
            return false;
        }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data){
                ...
                swal.fire("Aviso", "Ocurrió un error al intentar redirigir al ambiente seleccionado.", "warning");
            }
        },
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data){
                ...
                swal.fire("Aviso", "Ocurrió un error al intentar redirigir al foro virtual.", "warning");
            }
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
...
function show_ajax_error_message(jqXHR, textStatus = false, errorThrown = false, recarga_pagina = false){
                ...
                if(jqXHR.responseText){
                    ...
                    if(jqXHR.responseText.length > 1000){
                        ...
                        if(jqXHR.responseText == 'Sesión de usuario caducada'){
                            swal.fire("Aviso", jqXHR.responseText, "warning").then(function(){
    ...
    if(recarga_pagina){
        swal.fire("Aviso", mensaje, "warning").then(function(){location.reload();});
    }else{
...
function marcar_notificaciones_leidas(){
    $.ajax({
        ...
        error:function(){
            swal.fire("Aviso", "Ocurrió un error, intentelo nuevamente más tarde" , "warning");
        }
    });
}
...
function enviar_notificacion(){
    ...
    if(!user_emisor_id || !notif_destinatario || !asunto || !mensaje){
        swal.fire("Aviso", "Ocurrió un error, recargue la página e intente nuevamente", "warning");
        return false;
    }
    ...
    $.ajax({
        ...
        dataType: "JSON",
        success: function(data){
            swal.fire("Aviso", "Notificación enviada" , "success");
            $('#form-notif')[0].reset();
    ...
    if (fecha_validar.match(reg)){
        if(valor_min){
            ...
            if(time_fecha_form < time_fecha_actual){
                swal.fire("Aviso", "La fecha debe ser mayor a la actual", "warning");
                $(campo_fecha).val('');
                return false;
            }else{
                return true;
        ...
        swal.fire("Aviso", "Debe ingresar la fecha en formato dd/mm/yyyy", "warning");
        $(campo_fecha).val('');
        return false;
    }
}